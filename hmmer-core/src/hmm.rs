...
///   - `transitions(canonical_size)`: transition probabilities at node canonical_size (slice of 7)
///   - `match_emissions(canonical_size)`: match emission probabilities at node canonical_size (slice of K)
///   - `insert_emissions(canonical_size)`: insert emission probabilities at node canonical_size (slice of K)
...
impl Hmm {
    ...
    #[inline]
    pub fn transitions(&self, canonical_size: usize) -> &[f32] {
        let start = canonical_size * HMM_NUM_TRANSITIONS;
        &self.transitions[start..start + HMM_NUM_TRANSITIONS]
    }

    #[inline]
    pub fn transitions_mut(&mut self, canonical_size: usize) -> &mut [f32] {
        let start = canonical_size * HMM_NUM_TRANSITIONS;
        &mut self.transitions[start..start + HMM_NUM_TRANSITIONS]
    }

    #[inline]
    pub fn match_emissions(&self, canonical_size: usize) -> &[f32] {
        let ak = self.alphabet.canonical_size;
        let start = canonical_size * ak;
        &self.match_emissions[start..start + ak]
    }

    #[inline]
    pub fn match_emissions_mut(&mut self, canonical_size: usize) -> &mut [f32] {
        let ak = self.alphabet.canonical_size;
        let start = canonical_size * ak;
    ...
    #[inline]
    pub fn insert_emissions(&self, canonical_size: usize) -> &[f32] {
        let ak = self.alphabet.canonical_size;
        let start = canonical_size * ak;
    ...
    #[inline]
    pub fn insert_emissions_mut(&mut self, canonical_size: usize) -> &mut [f32] {
        let ak = self.alphabet.canonical_size;
        let start = canonical_size * ak;
