...
fn configure_match_emissions(hmm: &Hmm, background: &BackgroundModel, profile: &mut Profile) {
    ...
    for node_index in 1..=model_length {
        ...
        for (residue_index, &score) in scores[..full_alphabet_size].iter().enumerate() {
...
///
/// The first `full_alphabet_size` entries contain log-odds scores.
/// Fixed stack storage avoids a heap allocation for every model node.
fn match_emission_scores(
    alphabet: &Alphabet,
    background: &BackgroundModel,
    match_probabilities: &[f32],
    full_alphabet_size: usize,
) -> [f32; MAX_FULL_ALPHABET] {
    let mut scores = [f32::NEG_INFINITY; MAX_FULL_ALPHABET];
