...
//! two passes:
//!
//! 1. **OA fill** ([`OaDecoder::decode`]) — a DP recurrence analogous to Viterbi,
...
//! still making forbidden paths effectively unreachable. This deliberately
//! matches HMMER 3.4's `TSCDELTA` convention; it is a tiny-positive
//! approximation rather than an exact zero-weight transition.

use crate::hmmer_core::config::{PROFILE_NUM_TRANSITIONS, PTsc, TraceStateType};
use crate::hmmer_core::errors::HmmerError;
use crate::hmmer_core::profile::Profile;
use crate::hmmer_core::score_matrix::{
    BEGIN_STATE, C_STATE, EXIT_STATE, J_STATE, N_STATE, ScoreMatrix,
};
...
/// Main-state source of an E cell, retained per sequence row for O(1) traceback.
...
///
/// Returns the maximum OA score reaching E and the state/node that supplied it.
fn fill_main_nodes(
    i: usize,
    dp: &mut ScoreMatrix,
    pp: &ScoreMatrix,
    ...
    let mut exit_source = ExitSource {
        state: TraceStateType::Match,
        node_index: 1,
    };
    let mut exit_source_score = f32::NEG_INFINITY;
    ...
    for k in 1..num_nodes {
        ...
        if exit_gate == 1.0 && mm >= exit_source_score {
            exit_source_score = mm;
    ...
    if exit_gate == 1.0 {
        // Match wins a same-node M/D tie, while later match nodes win ties with
        // earlier nodes. These are HMMER's select_e() rules.
        if mm >= exit_source_score {
            exit_source_score = mm;
            exit_source = ExitSource {
                state: TraceStateType::Match,
        ...
        if dm > exit_source_score {
            exit_source = ExitSource {
                state: TraceStateType::Delete,
                node_index: num_nodes,
            };
        }
    } else {
...
impl OaDecoder {
    ...
    pub fn new(num_nodes: usize) -> Self {
        OaDecoder {
            // Keep the known model width, but allocate no speculative sequence
            // row; the first decode grows to the actual envelope length.
            dp: ScoreMatrix::new(num_nodes, 0).unwrap(),
    ...
    pub fn decode(
        ...
        let score =
            optimal_accuracy_fill(profile, posterior, &mut self.dp, &mut self.exit_sources)?;
        let trace = oa_trace(profile, posterior, &self.dp, &self.exit_sources)?;
...
fn optimal_accuracy_fill(
    ...
    posterior: &ScoreMatrix,
    dp: &mut ScoreMatrix,
    exit_sources: &mut Vec<ExitSource>,
    ...
    for (i, exit_source_slot) in exit_sources
        .iter_mut()
        .enumerate()
        .take(seq_len + 1)
        .skip(1)
    {
        ...
        let (x_e, exit_source) =
            fill_main_nodes(i, dp, posterior, &td, prev_b, exit_gate, num_nodes);
        *exit_source_slot = exit_source;
...
fn oa_trace(
    ...
    let mut cursor = TracebackCursor {
        sequence_position: seq_len,
        node_index: 0,
    };
    ...
    let terminal_position = trace_coordinate(seq_len, "sequence position")?;
    trace.append_with_pp(TraceStateType::Terminate, 0, terminal_position, 0.0);
    trace.append_with_pp(TraceStateType::CTerminal, 0, terminal_position, 0.0);

    let mut sprv = TraceStateType::CTerminal;

    while sprv != TraceStateType::Start {
        let (scur, next_cursor) =
            dispatch_traceback_state(posterior, dp, tsc, &special_td, exit_sources, sprv, cursor)?;
        cursor = next_cursor;
        ...
        let pp = get_postprob(
            ...
            sprv,
            cursor.node_index,
            cursor.sequence_position,
        );
        trace.append_with_pp(
            scur,
            trace_coordinate(cursor.node_index, "model node")?,
            trace_coordinate(cursor.sequence_position, "sequence position")?,
            pp,
        );

        // N/J/C self-loops are annotated at their emitted residue before the
        // cursor retreats, matching p7_GOATrace's deferred decrement.
        if is_special_self_loop(scur, sprv) {
            cursor = cursor.retreat(1, 0, scur)?;
        }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct TracebackCursor {
    sequence_position: usize,
    node_index: usize,
}

impl TracebackCursor {
    fn retreat(
        self,
        ...
        let sequence_position = self
            .sequence_position
            .checked_sub(sequence_positions)
            .ok_or_else(|| HmmerError::TracebackFailed {
                state: state.as_str().into(),
                reason: "sequence cursor moved before position 0".into(),
            })?;
        let node_index = self.node_index.checked_sub(model_nodes).ok_or_else(|| {
            HmmerError::TracebackFailed {
                state: state.as_str().into(),
                reason: "model cursor moved before node 0".into(),
            }
        })?;
        Ok(Self {
            sequence_position,
...
fn trace_coordinate(value: usize, name: &str) -> Result<i32, HmmerError> {
    i32::try_from(value).map_err(|_| HmmerError::Range(format!("{name} exceeds i32::MAX")))
}

#[inline]
fn is_special_self_loop(scur: TraceStateType, sprv: TraceStateType) -> bool {
    scur == sprv
        && matches!(
            scur,
            TraceStateType::NTerminal | TraceStateType::Jump | TraceStateType::CTerminal
        )
}
...
fn dispatch_traceback_state(
    ...
    dp: &ScoreMatrix,
    tsc: &[f32],
    special_td: &SpecialTransitionDeltas,
    exit_sources: &[ExitSource],
    sprv: TraceStateType,
    cursor: TracebackCursor,
) -> Result<(TraceStateType, TracebackCursor), HmmerError> {
    let i = cursor.sequence_position;
    let k = cursor.node_index;

    // Select the predecessor and return its coordinates explicitly. Emitting
    // main states retreat here; special-state self-loops retreat after their
    // posterior annotation is appended.
    match sprv {
        TraceStateType::Match => Ok((select_m(tsc, dp, i, k), cursor.retreat(1, 1, sprv)?)),
        TraceStateType::Delete => Ok((select_d(tsc, dp, i, k), cursor.retreat(0, 1, sprv)?)),
        TraceStateType::Insert => Ok((select_i(tsc, dp, i, k), cursor.retreat(1, 0, sprv)?)),
        // N with i=0 means we have reached the true start of the sequence.
        TraceStateType::NTerminal => Ok((
            if i == 0 {
        ...
        TraceStateType::CTerminal => Ok((select_c(special_td, posterior, dp, i), cursor)),
        TraceStateType::Jump => Ok((select_j(special_td, posterior, dp, i), cursor)),
        TraceStateType::End => {
            let source =
                exit_sources
                    .get(i)
                    .copied()
                    .ok_or_else(|| HmmerError::TracebackFailed {
                        state: sprv.as_str().into(),
                        reason: format!("missing cached exit source for sequence row {i}"),
                    })?;
            Ok((
                source.state,
                TracebackCursor {
        ...
        TraceStateType::Begin => Ok((select_b(special_td, dp, i), cursor)),
...
fn select_c(
    ...
    let p_loop = td.c_loop * (dp.special_score(i - 1, C_STATE) + pp.special_score(i, C_STATE));
    let p_exit = td.e_move * dp.special_score(i, EXIT_STATE);
    if p_loop > p_exit {
...
fn select_j(
    ...
    let p_loop = td.j_loop * (dp.special_score(i - 1, J_STATE) + pp.special_score(i, J_STATE));
    let p_exit = td.e_loop * dp.special_score(i, EXIT_STATE);
    if p_loop > p_exit {
        TraceStateType::Jump
    } else {
        TraceStateType::End
...
/// inter-domain region).
fn select_b(td: &SpecialTransitionDeltas, dp: &ScoreMatrix, i: usize) -> TraceStateType {
    let p_n = td.n_move * dp.special_score(i, N_STATE);
    let p_j = td.j_move * dp.special_score(i, J_STATE);
    if p_n > p_j {
        TraceStateType::NTerminal
    } else {
        TraceStateType::Jump
...
mod tests {
    ...
    fn one_residue_posterior(num_nodes: usize) -> ScoreMatrix {
        let mut posterior = ScoreMatrix::new(num_nodes, 1).unwrap();
        posterior.resize(num_nodes, 1).unwrap();
        posterior.fill_main_row(0, f32::NEG_INFINITY);
        posterior.fill_main_row(1, 0.0);
        posterior.set_special_row(0, [0.0; 5]);
        posterior.set_special_row(1, [0.0; 5]);
        posterior
    }

    #[test]
    fn one_node_one_residue_decode_has_expected_trace_and_score() {
        let mut profile = Profile::new(1, &Alphabet::amino());
        profile.mode = SearchMode::Local;
        let mut posterior = one_residue_posterior(1);
        posterior.set_match_score(1, 1, 1.0);

        let result = OaDecoder::new(1).decode(&profile, &posterior).unwrap();
        let states: Vec<_> = result.trace.steps.iter().map(|step| step.state).collect();
        ...
        assert_eq!(result.trace.steps[3].node_index, 1);
        assert_eq!(result.trace.steps[3].sequence_position, 1);
        assert!((result.score - 1.0).abs() < 1e-6);
        assert!((result.trace.get_expected_accuracy() - result.score).abs() < 1e-6);
    }

    #[test]
    fn fill_caches_local_and_glocal_exit_sources_with_upstream_ties() {
        let num_nodes = 3;
        let mut posterior = one_residue_posterior(num_nodes);
        posterior.set_match_score(1, 1, 0.8);
        posterior.set_match_score(1, 2, 0.2);
        posterior.set_match_score(1, 3, 0.1);
        let td = vec![1.0; num_nodes * PROFILE_NUM_TRANSITIONS];

        for (exit_gate, expected_state, expected_node) in [
            (1.0, TraceStateType::Match, 1),
            (0.0, TraceStateType::Delete, num_nodes),
        ] {
            let mut dp = ScoreMatrix::new(num_nodes, 1).unwrap();
            dp.resize(num_nodes, 1).unwrap();
            dp.fill_main_row(0, f32::NEG_INFINITY);

            let (_, source) =
                fill_main_nodes(1, &mut dp, &posterior, &td, 0.0, exit_gate, num_nodes);
            assert_eq!(source.state, expected_state);
            assert_eq!(source.node_index, expected_node);
        }
    }

    #[test]
    fn special_and_main_predecessor_ties_match_upstream_rules() {
        let mut dp = ScoreMatrix::new(2, 1).unwrap();
        dp.resize(2, 1).unwrap();
        dp.main.fill(1.0);
        dp.special.fill(1.0);
        let posterior = one_residue_posterior(2);
        let td = SpecialTransitionDeltas {
            n_loop: 1.0,
            n_move: 1.0,
            j_loop: 1.0,
            j_move: 1.0,
        ...
        let transitions = vec![1.0; 2 * PROFILE_NUM_TRANSITIONS];

        assert_eq!(select_d(&transitions, &dp, 1, 1), TraceStateType::Match);
        assert_eq!(select_i(&transitions, &dp, 1, 1), TraceStateType::Match);
        assert_eq!(select_c(&td, &posterior, &dp, 1), TraceStateType::End);
        assert_eq!(select_j(&td, &posterior, &dp, 1), TraceStateType::End);
        assert_eq!(select_b(&td, &dp, 1), TraceStateType::Jump);
    }

    #[test]
    fn forbidden_transition_delta_matches_hmmer_tscdelta() {
        assert_eq!(to_delta(f32::NEG_INFINITY), f32::MIN_POSITIVE);
        assert_eq!(to_delta(-42.0), 1.0);
        assert_eq!(to_delta(0.0), 1.0);
    }

    #[test]
    fn cursor_retreat_reports_invalid_traceback_coordinates() {
        let cursor = TracebackCursor {
            sequence_position: 0,
            node_index: 0,
        };
        assert!(cursor.retreat(1, 0, TraceStateType::Match).is_err());
        assert!(cursor.retreat(0, 1, TraceStateType::Delete).is_err());
    }
}
